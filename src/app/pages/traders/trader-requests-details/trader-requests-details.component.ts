...
export class TraderRequestsDetailsComponent {
  ...
  // }
  API_getItemDetails() {
    this.ApiService.get(`Traders/requests/${this.getID}`).subscribe(
      (res: any) => {
        if (res.data) {
          this.form.patchValue({
            ...res.data,
            ...
            logitude:
              res.data.addresses[0].longitude ?? res.data.addresses[0].logitude,
          });
          ...
          this.adress = [
            {
              ...
              latitude: res.data.addresses[0].latitude,
              logitude:
                res.data.addresses[0].longitude ??
                res.data.addresses[0].logitude,
              userId: Number(localStorage.getItem('userId')) || 0,
            },
          ];
          console.log(
            'TraderDetailsComponent  this.ApiService.get    this.adress:',
            this.form.value,
          );
  ...
  openDialog(value: string) {
    ...
    if (this.action == 'approve') this.header = 'trader_request.q_approve';
    else this.header = 'trader_request.q_reject';
  }

  approve() {
    this.ApiService.putWithId('Traders/approve', this.getID).subscribe(
      (res: any) => {
