...
export class Validations {
  ...
  static phoneValidator(countries: any[], errorMessage?: string): ValidatorFn {
    return (control: AbstractControl<string>) => {
      ...
      if (
        (phoneValue.startsWith('05') && phoneValue.length === 10) ||
        (phoneValue.startsWith('5') && phoneValue.length === 9)
      ) {
        matchingCountry = countries.find(
          (c) => norm(c.phoneCode || '') === '966',
        );
      }
      // Oman: 09/07/02 + 9 digits
        ...
        (phoneValue.startsWith('09') ||
          ...
          phoneValue.startsWith('02')) &&
        phoneValue.length === 9
      ) {
        matchingCountry = countries.find(
          (c) => norm(c.phoneCode || '') === '968',
        );
      }
